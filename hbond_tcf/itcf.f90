...
! DUMP FILE RELATED VARIABLE
    ...
    REAL                                         :: dump_freq = 5.0   ! default 5ps per frame
...
! TCF RELATED VARIABLES
    INTEGER(KIND=1), ALLOCATABLE, DIMENSION(:,:,:) :: sij_an1, sij_an2F
...
! TIME RELATED VARIABLES
    ...
    REAL, ALLOCATABLE, DIMENSION(:)    :: run_time
    INTEGER                            :: tot_frames, skip_frames, proc_frames
! BOOK KEEPING VARIABLES
    
    INTEGER            :: i=0, j=0, k=0, l=0, m=0, ios=0, temp1=0, temp2=0, itemp=0
...
!********** END SKIP FRAMES **********
    ...
    sij_an1(:,:,:)  = 0
    sij_an2F(:,:,:) = 0
    ALLOCATE(run_time(0:proc_frames-1))
...
!********** PROCESS FRAMES **********
    ...
    DO i = 1, proc_frames
        ...
        run_time(i-1) = dump_freq*REAL(frame_no-frame_zero)

    END DO
    DEALLOCATE(id, mol, at_type, r)
    CALL close_file(file_hno)

    ALLOCATE(denom_an1(proc_frames), denom_an2F(proc_frames))
    ALLOCATE(dnumer_an1(0:proc_frames-1,proc_frames), dnumer_an2F(0:proc_frames-1,proc_frames))
    
    denom_an1(:)     = 0
    denom_an2F(:)    = 0
    dnumer_an1(:,:)  = 0.D0
    dnumer_an2F(:,:) = 0.D0
    ! NEED INTERLEAVED OR DYNAMIC SCHEDULING AS LOAD IS IMBALENCED
    ! PRIVATE VARIABLES --
        ! LOOP RELATED VARIABLES --
            ! t0, j, k, tf, delt
    ! SHARED VARIABLES --
        ! USER DEFINED PARAMETERS --
            ! proc_frames, no_ring, no_an1, no_an2F
        ! INPUT DATA STRUCTURES --
            ! sij_an1, sij_an2F
        ! OUTPUT DATA STRUCUTRES --
            ! denom_an1, denom_an2F, dnumer_an1, dnumer_an2F
!$OMP PARALLEL DO SCHEDULE(STATIC, 1) DEFAULT(NONE) PRIVATE(t0,j,k,tf,delt) &
!$OMP& SHARED(proc_frames,no_ring,no_an1,no_an2F,sij_an1,sij_an2F) &
!$OMP& SHARED(denom_an1,denom_an2F,dnumer_an1,dnumer_an2F,run_time) 
    DO t0 = 1, proc_frames
        PRINT '(A,F11.3,A)', "***** ESTIMATING TCF AT T0 : ", run_time(t0-1), " ps"

        ! ESTIMATING THE DENOMINATOR AT TIME T0
        DO j = 1, no_ring
            DO k = 1, no_an1
                ! denom_an1(t0) = denom_an1(t0) + sij_an1(k,j,t0)**2
                ! sij_an1(k,j,t0)**2 == sij_an1(k,j,t0) WHEN sij_an1(k,j,t0) IS EITHER 1 OR 0
                denom_an1(t0) = denom_an1(t0) + sij_an1(k,j,t0)
            END DO

            DO k = 1, no_an2F
                ! denom_an2F(t0) = denom_an2F(t0) + sij_an2F(k,j,t0)**2
                ! sij_anF(k,j,t0)**2 == sij_anF(k,j,t0) WHEN sij_an2F(k,j,t0) IS EITHER 1 OR 0
                denom_an2F(t0) = denom_an2F(t0) + sij_an2F(k,j,t0)
            END DO
        END DO

        IF(t0 == 1) PRINT *, "DENOMINATOR : ", denom_an1(t0)

        DO tf = t0, proc_frames
            ! delt goes from 0 to proc_frames-1 when t0=1 and
            !                0 to 0             when t0=proc_frames
            delt = tf-t0
            DO j = 1, no_ring
                DO k = 1, no_an1
                    dnumer_an1(delt,t0) = dnumer_an1(delt,t0) + REAL(sij_an1(k,j,t0)*sij_an1(k,j,tf),KIND=REAL64)
                END DO
                DO k = 1, no_an2F
                    dnumer_an2F(delt,t0) = dnumer_an2F(delt,t0) + REAL(sij_an2F(k,j,t0)*sij_an2F(k,j,tf),KIND=REAL64)
                END DO
            END DO
        END DO
        IF(t0 == 1) PRINT *, "NUMERATOR AT DELT 0 : ", dnumer_an1(0,t0)
        ! NORMALIZE NUMERATORS TO GET HBNOND-TCF FOR AN INITIAL FRAME T0
...
!$OMP END PARALLEL DO
    ...
    ALLOCATE(ci_an1(0:proc_frames-1), ci_an2F(0:proc_frames-1))
    ci_an1(:)  = 0.D0
    ci_an2F(:) = 0.D0

    PRINT *, "***** AVERAGING TCF *****"

    DO delt = 0, proc_frames-1
        mycount = 0
    ...
    12 FORMAT(F11.3, 2X, E15.8)
    DO delt = 0, proc_frames-1
        WRITE(50,12) run_time(delt), ci_an1(delt)
        WRITE(60,12) run_time(delt), ci_an2F(delt)
    END DO
